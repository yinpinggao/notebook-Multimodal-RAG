...
- **Database**: SeekDB (MySQL) async driver
- **AI Providers**: Esperanto library (8+ providers: OpenAI, Anthropic, Google, Groq, Ollama, Mistral, DeepSeek, xAI)
- **Job Queue**: seekdb-commands for async jobs (podcasts)
- **Logging**: Loguru
- **Validation**: Pydantic v2
- **Testing**: Pytest

### Database
- **SeekDB (MySQL)**: Relational database with built-in embedding storage and vector search
...
- SeekDB async driver with connection pooling
...
- **SeekDB graph model**: Records with relationships and embeddings
...
- **SeekDB must be running**: API fails without database connection
...
- **[database/CLAUDE.md](database/CLAUDE.md)**: SeekDB operations, migrations, async patterns
...
1. Create `migrations/XXX_description.py`
2. Write SQL migration using SeekDB business store
