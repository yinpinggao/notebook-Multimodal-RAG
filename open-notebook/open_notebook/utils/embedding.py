...
async def generate_embeddings(
    ...
    for batch_idx in range(total_batches):
        ...
        for attempt in range(1, EMBEDDING_MAX_RETRIES + 1):
            try:
                embed_kwargs = {"task_type": task_type} if task_type else {}
                batch_embeddings = await embedding_model.aembed(batch, **embed_kwargs)
...
async def generate_embedding(
    ...
    if len(text) <= CHUNK_SIZE:
        ...
        embeddings = await generate_embeddings(
            [text], command_id=command_id, task_type=task_type
        )
        return embeddings[0]
    ...
    if len(chunks) == 1:
        ...
        embeddings = await generate_embeddings(
            chunks, command_id=command_id, task_type=task_type
        )
        return embeddings[0]
