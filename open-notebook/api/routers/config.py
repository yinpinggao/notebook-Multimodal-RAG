...
async def check_database_health() -> dict:
    """
    Check if the primary datastore is reachable using a lightweight ping.

    Returns:
        dict with 'status' ("online" | "offline") and optional 'error'
    """
    try:
        if await asyncio.wait_for(seekdb_client.ping(), timeout=2.0):
            return {"status": "online"}
        return {"status": "offline", "error": "SeekDB ping failed"}
...
async def check_seekdb_health() -> dict:
    if not seekdb_is_configured():
    ...
    try:
        ...
        logger.warning(f"SeekDB health check failed: {e}")
        return {"status": "offline", "error": str(e)}
...
async def get_config(request: Request):
    ...
    seekdb_health = await check_seekdb_health()
    seekdb_status = seekdb_health["status"]
    ...
    return {
        ...
        "dbStatus": db_status,
        "seekdbConnected": seekdb_status == "online",
        "seekdbStatus": seekdb_status,
    }
