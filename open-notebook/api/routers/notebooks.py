...
async def get_notebooks(
    ...
    try:
        ...
        result = await seekdb_business_store.notebook_rows(order_by=order_by)
...
async def get_notebook(notebook_id: str):
    ...
    try:
        ...
        row = await seekdb_business_store.notebook_row(notebook_id)
...
async def add_source_to_notebook(notebook_id: str, source_id: str):
    ...
    try:
        ...
        existing_ref = await seekdb_business_store.relation_exists(
            "reference",
            str(source_id),
            str(notebook_id),
        )
        ...
        if not existing_ref:
            await seekdb_business_store.create_relation(
                "reference", str(source_id), str(notebook_id)
            )
            if use_seekdb_for_search() and source.full_text and source.full_text.strip():
                try:
                    ...
                    logger.warning(
                        f"Failed to queue SeekDB source reindex for {source_id}: {e}"
                    )
    ...
    try:
        ...
        if use_seekdb_for_search():
            try:
                ...
                logger.warning(
                    f"Failed to queue SeekDB source reindex after unlink for {source_id}: {e}"
                )
