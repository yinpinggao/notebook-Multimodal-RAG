...
async def get_sources(
    ...
    try:
        ...
        if notebook_id:
            ...
            result = await seekdb_business_store.source_list_rows(
                notebook_id=notebook_id,
                limit=limit,
                offset=offset,
                sort_by=sort_by,
        ...
        for row in result:
            ...
            if command:
                ...
                try:
                    source_obj = await Source.get(row["id"])
                    if source_obj:
                        status = await source_obj.get_status()
                        processing_info = await source_obj.get_processing_progress()
                except Exception:
                    status = "unknown"
            ...
            if use_seekdb_for_search():
                try:
                    source_obj = await Source.get(row["id"])
                    if source_obj:
                        embedded_chunks, page_count = await _get_source_index_stats(
                            source_obj
                        )
...
async def create_source(
    ...
    try:
        ...
        if source_data.async_processing:
            ...
            # Create minimal source record - let the database generate the ID
            source = Source(
            ...
            try:
                ...
                # Create source record - let the database generate the ID
                source = Source(
                    title=source_data.title or "Processing...",
                    topics=[],
                )
...
async def get_source(source_id: str):
    ...
    try:
        ...
        notebook_ids = await seekdb_business_store.get_source_notebook_ids(
            source.id or source_id
        )
...
async def update_source(source_id: str, source_update: SourceUpdate):
    ...
    try:
        ...
        await source.save()
        if use_seekdb_for_search() and source.full_text and source.full_text.strip():
            try:
                await source.vectorize()
            except Exception as e:
                logger.warning(f"Failed to queue SeekDB source reindex for {source_id}: {e}")
...
async def retry_source_processing(source_id: str):
    ...
    try:
        ...
        notebook_ids = await seekdb_business_store.get_source_notebook_ids(source_id)
