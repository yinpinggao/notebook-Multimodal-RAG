...
class TestModelManager:
    ...
    @pytest.mark.asyncio
    async def test_get_model_falls_back_to_latest_valid_provider_credential(self):
        """If a linked credential is unreadable, use the latest valid provider credential."""
        ...
        with (
            patch("open_notebook.ai.models.Model.get", new=AsyncMock(return_value=model)),
            patch(
                "open_notebook.domain.credential.Credential.get_latest_valid_for_provider",
                new=AsyncMock(return_value=fallback_credential),
            ),
            patch(
                "open_notebook.ai.models.create_runtime_model",
                return_value="runtime-model",
            ) as create_runtime_model,
        ):
            result = await manager.get_model("model:test")

        assert result == "runtime-model"
        create_runtime_model.assert_called_once_with(
            provider="deepseek",
...
class TestCredentialDomain:
    """Test suite for credential-specific helpers."""

    def test_decode_rows_skips_unreadable_credentials(self):
        """A single unreadable credential should not fail the whole list."""
        valid = Credential(
            id="credential:ok",
            name="Valid",
            provider="deepseek",
        ...
        with patch.object(
            Credential,
            "_from_db_row",
            side_effect=[ValueError("bad token"), valid],
        ):
            decoded = Credential._decode_rows(
                [{"id": "credential:bad"}, {"id": "credential:ok"}],
                context="unit test",
            )
...
class TestSourceDomain:
    ...
    def test_source_command_field_parsing(self):
        """Test command field parsing for string job IDs."""
