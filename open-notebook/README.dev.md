...
```bash
# Start datastore and queue
make database

# Start all services (SeekDB + Redis + API + Worker + Frontend)
make start-all
```

### What This Does

1. Starts SeekDB and Redis in Docker
2. Starts FastAPI backend (port 5055)
3. Starts ARQ background worker
...
1. Create migration file: `migrations/N_description_up.py`
2. Write schema changes in Python migration functions (up() function applies, down() function rolls back)
3. (Optional) Create rollback: `migrations/N_description_down.py`
...
# Check database
docker compose ps seekdb redis

# View logs
docker compose logs seekdb redis
...
```bash
# Verify SeekDB and Redis are running
docker compose ps seekdb redis

# Check connection settings in .env
cat .env | grep OPEN_NOTEBOOK_SEEKDB_DSN
```
...
```bash
# .env file
OPEN_NOTEBOOK_SEEKDB_DSN=mysql://root:SeekDBRoot123%21@localhost:2881/open_notebook_ai
OPEN_NOTEBOOK_AI_CONFIG_BACKEND=seekdb
OPEN_NOTEBOOK_SEARCH_BACKEND=seekdb
OPEN_NOTEBOOK_JOB_BACKEND=arq
OPEN_NOTEBOOK_REDIS_URL=redis://localhost:6379/0
...
2. **Keep SeekDB running** between sessions (`make database`)
