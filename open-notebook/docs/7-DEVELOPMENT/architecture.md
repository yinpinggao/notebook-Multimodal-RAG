...
└────────────────┬────────────────────────────────────────┘
         ...
         └───────┬───────┘
                 ...
                 ▼
         ┌───────────────┐
         │   SeekDB   │  ← Database (internal, auto-configured)
         │   (Port 3030) │
         └───────────────┘
```
...
```
HTTP Request → Router → Service → Domain/Repository → SeekDB
                                       ↓
                                  LangGraph (optional)
                                       ↓
...
### Layer 3: Database (SeekDB @ port 8000)

**Purpose**: Graph database with built-in vector embeddings, semantic search, and relationship management.

**Technology Stack**:
- **Database**: SeekDB (multi-model, ACID transactions)
- **Query Language**: SeekQL (SQL-like syntax with graph operations)
- **Async Driver**: Async Rust client for Python
- **Migrations**: Python migration files in `/migrations/` (auto-run on API startup)
...
- Embeddings stored natively in SeekDB
...
- Flexible SeekQL (SQL-like syntax)
...
**Key Features**:
- Message history persisted in SeekDB (SqliteSaver checkpoint)
...
- `repo_query()`: Execute SeekQL queries
...
For async background tasks (source processing), use seekdb-Commands job queue:
...
1. **SeekQL queries** (similar to SQL)
...
- Numbered Python files (001_up.py, 001_down.py, etc)
- Tracked in `_sbl_migrations` table
- Rollback via `_down.py` files (manual)
...
SeekDB for graph + vector search in one system (no external dependencies).
...
- **SeekDB must be running** before starting API (connection test in lifespan)
- **Auth middleware is basic** (password-only); upgrade to OAuth/JWT for production

### Database Operations

- **Record IDs use SeekDB syntax** (table:id format, e.g., "notebook:abc123")
...
1. **Connection Pooling**: SeekDB async driver with configurable pool size
...
- **Database Queries**: SeekDB metrics available via admin UI
...
- **Database**: SeekDB container or managed service
