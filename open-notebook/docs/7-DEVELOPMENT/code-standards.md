...
Use the business store consistently:

```python
from open_notebook.seekdb import seekdb_business_store

# Create records
async def create_notebook(data: dict) -> dict:
    """Create a new notebook record."""
    return await seekdb_business_store.create_entity("notebook", data)

# Query with parameters
async def find_notebooks_by_user(user_id: str) -> list[dict]:
    """Find notebooks for a specific user."""
    return await seekdb_business_store.list_entities(
        "notebook",
        filters={"user_id": user_id}
    )

# Update records
async def update_notebook(notebook_id: str, data: dict) -> dict:
    """Update a notebook record."""
    return await seekdb_business_store.update_entity(
        "notebook", notebook_id, data
    )
```

### Schema Management

Schema is managed via the SeekDB client:

```python
from open_notebook.seekdb import seekdb_client

# Schemas are defined in open_notebook/seekdb/business_store.py
# Run schema setup:
