...
function shouldPollSource(source?: Partial<SourceListResponse> & { status?: string; command_id?: string }) {
  if (!source) {
    return false
  }
  ...
  // After process_source completes, embed_source may still be running for a short
  // time. Keep polling while the source is not yet embedded so the UI catches up
  // without requiring a manual refresh.
  if (status === 'completed' && !!source.command_id && !source.embedded) {
    return true
  }
...
export function useSources(notebookId?: string) {
  return useQuery({
    ...
    refetchInterval: (query) => {
      ...
      return data?.some(shouldPollSource) ? SOURCE_POLL_INTERVAL_MS : false
    },
  })
}
...
export function useNotebookSources(notebookId: string) {
  ...
  const query = useInfiniteQuery({
    ...
    refetchInterval: (query) => {
      const data = query.state.data?.pages.flatMap(page => page.sources) ?? []
      return data.some(shouldPollSource) ? SOURCE_POLL_INTERVAL_MS : false
    },
  })
...
export function useSource(id: string) {
  return useQuery({
    ...
    refetchOnWindowFocus: true, // Refetch when user comes back to the tab
    refetchInterval: (query) => {
      const data = query.state.data as SourceResponse | undefined
      return shouldPollSource(data) ? SOURCE_POLL_INTERVAL_MS : false
    },
  })
}
